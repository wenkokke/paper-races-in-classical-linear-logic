...
\wen{
  In \cref{sec:cp}, we will discuss classical processes~\cite[CP]{wadler2012},
  the process calculus which we are extending. 
  In \cref{sec:local-choice}, we will discuss some existing work which extends
  logic-inspired session typed process calculi with non-determinism.
}
...
bit of information (\tm{inl} or \tm{inr}) and branching based on the value of
that bit. 
\begin{center}
  \cpInfPlus1
  \cpInfWith
...
\wen{Add a note to \cref{sec:cp-multiplicatives} which explains the relationship
with deadlock freedom.}
...
\wen{Write bit on why Phil doesn't use structural congruence for anything,
  because he uses commutative conversions.}
