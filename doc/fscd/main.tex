...
of clients.  It would be more realistic if we could define serves that provided
for arbitrary numbers of clients.  This poses two problems: how would we define
such arbitrarily-interacting stateful processes, and how would we extend the
typing discipline of \nodcap to account for them without losing static
guarantees.

One approach to defining server processes would be to combine \nodcap with
structural recursion, such as the $\mu\text{CP}$ extension proposed by Lindley
...
quantifiers $\forall x A$ and $\exists x A$.  Defining a term structure
...
expect those instances to be potentially dependent.

With quantification over resource variables, we can give precise accounts in
\nodcap of both CP's exponentials and idealised servers and clients.  CP
...
$\exists n \take[n] A$, being of definite size but with each client being
