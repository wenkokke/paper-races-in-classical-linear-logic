...
with the elimination of the \textsc{Cut} rule. 
The eliminations of principal cuts can be found in Figure~\ref{fig:cp-cut-prin}.
The commucatitive conversions can be found in Figure~\ref{fig:cp-cut-cc}.
...
\end{prooftree}
The second rule, \textsc{Contract}, enables the contraction of server
interactions. This enables us to construct a server which has multiple
interactions of the same type across the same channel $x$:
\begin{prooftree}
...
\begin{prooftree}
  ...
  \SYM{}
  \BIC{$\seq[{ (\recv{x}{y}\john \mid \recv{x}{z}\mary) }]{
