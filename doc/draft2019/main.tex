...
\institute{%
  ...
  \email{wen.kokke@ed.ac.uk}, \email{wadler@inf.ed.ac.uk} \and
  University of Kansas, Lawrence, KS, USA,\\
  \email{garrett@ku.edu}}
\begin{document}
...
There are two versions of \dhcp: a version with delayed actions, introduced by Kokke \etal~\cite{kokke2019pacmpl}; and a version without delayed actions, introduced by Kokke \etal~\cite{kokke2018tlla}, referred to as \hcp. In this work, we will base ourselves on the latter, as the former does not yet have reduction semantics.
...
\begin{definition}[Terms]\label{def:hcp-terms}
  ...
  \]
\end{definition}\noindent
The variables \tm{x}, \tm{y}, \tm{z} and \tm{w} range over channel names. Occasionally, we use $\tm{a}$, $\tm{b}$, and $\tm{c}$ to range over \emph{free} channel names. The construct \tm{\cpLink{x}{y}} links two channels~\cite{sangiorgi1996,boreale1998}, forwarding messages received on \tm{x} to \tm{y} and vice versa. The construct $\tm{\piNew{x}{P}}$ creates a new channel $\tm{x}$, and the construct $\tm{\piPar{P}{Q}}$ and composes two processes. In \tm{\piRecv{x}{y}{P}} and \tm{\piSend{x}{y}{P}}, round brackets denote input, square brackets denote output. We use bound output~\cite{sangiorgi1996}, meaning that both input and output bind a new name. 
...
\begin{prooftree}
  \AXC{$\seq[{ \ami }]{ \Gamma, \tmty{z}{\money^\bot}, \tmty{\sliceofcake}{\cake}}$}
  \SYM{\tens}
  \UIC{$\seq[{ \piSend{y}{\sliceofcake}{\ami} }]{ \Gamma, \tmty{z}{\money^\bot}, \tmty{y}{\cake}}$}
  \SYM{\parr}
  \UIC{$\seq[{ \piRecv{x}{z}{\piSend{y}{\sliceofcake}{\ami}} }]{ \Gamma, \tmty{x}{\money^\bot}, \tmty{y}{\cake}}$}
  \AXC{$\seq[{ \boe }]{ \Delta, \tmty{\bill}{\money}, \tmty{w}{\cake^\bot} }$}
  \SYM{\tens}
  \UIC{$\seq[{ \piSend{x}{\bill}{\boe} }]{ \Delta, \tmty{x}{\money}, \tmty{w}{\cake^\bot} }$}
  \SYM{\parr}
  \UIC{$\seq[{ \piRecv{y}{w}{\piSend{x}{\bill}{\boe}} }]{ \Delta, \tmty{x}{\money}, \tmty{y}{\cake^\bot} }$}
  \NOM{H-Mix}
  \BIC{$\seq[{
      (\piPar{\piRecv{x}{z}{\piSend{y}{\sliceofcake}{\ami}}}{\piRecv{y}{w}{\piSend{x}{\bill}{\boe}}}) }]{
  ...
  \NOM{Cut}
  \UIC{$\seq[{
      \piNew{y}{(\piPar{\piRecv{x}{z}{\piSend{y}{\sliceofcake}{\ami}}}{\piRecv{y}{w}{\piSend{x}{\bill}{\boe}}})}
    }]{
...
\begin{prooftree}
  ...
  \NOM{H-Mix}
  \BIC{$\seq[{ (\piPar{\cpSend{y}{x}{\ami}{\boe}}{\cpRecv{y}{x}{\store}}) }]{
      \Gamma, \Delta, \tmty{y}{\cake^\bot \tens \cake^\bot}
      \hsep
      \Theta, \tmty{y}{\cake \parr \cake} }$}
  \NOM{Cut}
  \UIC{$\seq[{ \cpCut{y}{\cpSend{y}{x}{\ami}{\boe}}{\cpRecv{y}{x}{\store}} }]{
      \Gamma, \Delta, \Theta }$}
\end{prooftree}
...
\begin{prooftree}
  \AXC{$\seq[{ \ami }]{ \Gamma, \tmty{y}{\cake^\bot} }$}
  \SYM{(\take[1]{})}
  \UIC{$\seq[{ \ncCnt{x}{y}{\ami} }]{ \Gamma, \tmty{y}{\take[1]{\cake^\bot}} }$}

  \AXC{$\seq[{ \boe }]{ \Delta, \tmty{y'}{\cake^\bot} }$}
  \SYM{(\take[1]{})}
  \UIC{$\seq[{ \ncCnt{x'}{y'}{\boe} }]{ \Delta, \tmty{x'}{\take[1]{\cake^\bot}} }$}
...
In this section, we will contranst our work with the more recent work on manifest sharing~\cite{balzer2017}.
